...
$source = "P:\FOTO SPETTACOLI\FOTO LAVORI\GERTRUDE (LE CRI)"
...
1..4 | ForEach-Object {
    ...
    $n = "{0:D2}" -f $i
    $srcCandidates = @(
        (Join-Path $source ("Gertrude (Le Cri) - Cristian Taraborrelli {0}.jpg" -f $i)),
        (Join-Path $source ("Gertrude (Le Cri) - Cristian Taraborrelli {0}.JPG" -f $i))
    )
    $src = $srcCandidates | Where-Object { Test-Path $_ } | Select-Object -First 1
    if (-not $src) { Write-Host "MISSING for index $i"; return }

    $out = Join-Path $dest "gertrude-le-cri-$n.jpg"
    ...
    Write-Host ("OK: gertrude-le-cri-{0}.jpg  {1}x{2}  {3:N0} KB" -f $n, $newW, $newH, $sz)
}
Write-Host "Done."
